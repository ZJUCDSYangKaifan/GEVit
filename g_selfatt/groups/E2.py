...
class E2(Group):
    ...
    def left_action_on_Rd(self, h: GroupElement, pp: torch.Tensor):
        if False not in (h == self.identity):
            ...
            if h[-1] == -1:
                Lhpp = torch.einsum("oi,ixy->oxy", self.matrix_mirror(), Lhpp).type(torch.float32)
    ...
    def left_action_on_H(self, h: GroupElement, pp_h: torch.Tensor):
        if False not in (h == self.identity):
            ...
            if h[-1] == -1:
                ...
                Lhpp_h = (Lhpp_h + self.num_elements // 2) % self.num_elements
    ...
    def matrix_mirror(self) -> torch.Tensor:

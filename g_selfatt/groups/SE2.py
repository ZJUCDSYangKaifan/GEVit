...
class SE2(Group):
    ...
    def construct_relative_positions(self):
        indices = torch.tensor([0]+[i for i in range(self.num_elements-1, 0, -1)])
        return torch.stack([self.left_action_on_H(-h, self.left_action_on_H(-h, indices)) for h in self.elements], dim=0)
